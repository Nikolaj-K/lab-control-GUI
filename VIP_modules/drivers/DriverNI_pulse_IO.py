...
################################################################################
### Create a list of zeros that can be used to specify "turn all pins off".
ZEROS = [0 for _ in hardware.range_NI_pins]

################################################################################
class DriverNI_pulse(DriverVIP):
    """This driver has only one active public method, namely 'send_pulse', which
    takes a session dictionary with specifications of which pins to turn on and
    does so.
    """

    def __init__(self, instr_name):
        super(DriverNI_pulse, self).__init__(instr_name)

        print "(DriverVISA.__init__("+instr_name+"))"
